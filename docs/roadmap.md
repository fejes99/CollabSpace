...
2. **First service skeleton — auth-workspace** _(complete)._ Spring Boot 4.0.6 + Java 25. `GET /actuator/health → 200 OK`. Multi-stage Docker build (eclipse-temurin:25-jdk-noble → eclipse-temurin:25-jre-noble). Deployed to ECS Fargate and reachable at the ALB DNS name.
3. **CI/CD pipeline — auth-workspace** _(complete)._ GitHub Actions workflow: test → Docker build (linux/amd64) → ECR push (`:<sha>`) → ECS deploy with ALB health-check stability wait. Triggered on push to `main` when `services/auth-workspace/**` or the workflow file changes. ECR tags are immutable; `:skeleton` was pushed once on the first run to bootstrap the Terraform-created ECS service and is not updated by CI.
4. **Remaining service skeletons.** document-service (TypeScript/Fastify), realtime-service (TypeScript/Fastify), AI assistant (Python/FastAPI), notification (Node.js Lambda). Each slots into the existing CI/CD pattern. Realtime is the exception: ECS-on-EC2 rather than Fargate, and the health signal is a WebSocket echo rather than an HTTP endpoint.
