...
3. **CI/CD pipeline — auth-workspace** _(complete)._ GitHub Actions workflow: test → Docker build (linux/amd64) → ECR push (`:skeleton` + `:<sha>`) → ECS deploy with ALB health-check stability wait. Triggered on push to `main` when `services/auth-workspace/**` or the workflow file changes.
