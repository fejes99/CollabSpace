...
### auth-workspace: token-refresh (2026-07)

- `POST /v1/auth/refresh` — reads the `refresh_token` cookie, looks it up by SHA-256 hash, and atomically rotates it (delete old row, insert new row, mint a new access token, all in one `@Transactional` service method) — deliberately mirrors the plan doc's requirement that a mid-rotation failure leaves the original cookie valid for retry. Distinguishes `401 auth/refresh-token-invalid` (missing/oversized/unknown cookie) from `401 auth/refresh-token-expired` (known but past `expires_at`) — two new rows added to `error-catalog.md`'s `auth/` block.
- The new access token re-derives `memberships` from `WorkspaceMembershipRepository` rather than copying the old token's claims forward — required for ADR-032's staleness invalidation to have any effect on a refreshed token.
- Found and fixed during review, before merge: the expired-token branch deleted the row and then threw, but plain `@Transactional` rolls back on any unchecked exception by default — silently undoing that delete. Fixed with `@Transactional(noRollbackFor = ExpiredRefreshTokenException.class)`. Caught by a new `RefreshExpiredTokenPersistenceIntegrationTest`, deliberately **not** `@Transactional` at the class level (same reasoning as `RefreshConcurrencyIntegrationTest` and ADR-034) — a test sharing the app code's transaction would "read its own writes" regardless of whether the delete actually committed, proving nothing either way.
- Found and fixed during manual smoke testing, before merge: the refresh cookie's `Set-Cookie` used `Path=/auth`, but `AuthController` is mounted at `/v1/auth` — per RFC 6265 path-matching, a real browser/curl never sends that cookie back to `/v1/auth/*` at all (it only matches `/auth` or `/auth/*`). This bug predates this PR (present in `login()`'s cookie since the original login endpoint) but had never been exercised, since no endpoint read the cookie back until this one. `MockMvc`'s test helper attaches cookies directly to the request without real path-matching, so all existing tests passed regardless — only a real-HTTP smoke test surfaced it. Fixed to `Path=/v1/auth`; `authentication.md`, this README, and the plan doc corrected to match. `LoginIntegrationTest`'s and `RefreshIntegrationTest`'s cookie-path assertions updated (they had been silently asserting the wrong value).
- Package reorg carried over from list-workspaces' pattern: `adapter/in/rest/auth/` split into `request/`/`response/`; `application/port/in/auth/` split into `command/`/`result/`/`usecase/`; `domain/exception/` split into `auth/`/`workspace/` subpackages (both has grown past a flat handful of files, same threshold as the workspace package reorg).
- `CurrentUserIdResolver` extracted from `WorkspaceController`'s private `currentUserId()` — no second copy needed once `AuthController` needed the same helper.
- Tests: unit (`RefreshTokenValidatorTest`, `AuthApplicationServiceTest` additions), integration (`RefreshIntegrationTest` — 8 cases including cookie rotation and membership re-derivation; `RefreshConcurrencyIntegrationTest` — same-token race, exactly one winner; `RefreshExpiredTokenPersistenceIntegrationTest` — the rollback-fix proof above). Verified locally (Docker + Testcontainers, 329 tests passing). Not yet verified on AWS.
