...
- **Self-directed** (a user creates a workspace, accepts an invite, or changes their own role): the mutating endpoint reissues a fresh access token synchronously in its response, or the client immediately calls `POST /v1/auth/refresh`. This applies regardless of whether the change is a grant or a revocation (e.g., demoting yourself) — no delay, ever.
