...
There is consequently no single "transfer ownership" action. Giving up admin status is two separate calls, both targeting a specific `userId` via the existing "change member role" capability: promote a named user to `admin`, then optionally demote yourself. This is safe by construction without needing an atomic combined endpoint — the last-admin invariant is checked at demote-time regardless of when it's called, so there is no window where the workspace can end up with zero admins even if the two calls happen far apart or only the first one is ever made.
