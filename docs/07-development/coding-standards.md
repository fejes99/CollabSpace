...
- **Flyway migrations: always name constraints and indexes explicitly.** Use `CONSTRAINT <name> UNIQUE (col)` / `CONSTRAINT <name> FOREIGN KEY ...` — never rely on Postgres auto-generated constraint names (e.g. `users_email_key`). Application code that catches `DataIntegrityViolationException` must check the constraint name string; auto-generated names change if the table is recreated, breaking the check silently. For indexes, use `CREATE INDEX idx_{table}_{column(s)} ON {table} ({column(s)})` — the `idx_` prefix plus table and column makes the purpose and scope unambiguous at a glance.
