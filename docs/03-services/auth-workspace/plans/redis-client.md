...
- `probes.enabled=true` also auto-creates `/actuator/health/readiness` — left at Spring Boot's default and intentionally unused, same as `liveness` (see note below)
- **Correction from initial draft:** there is currently no ALB `health_check_path` to change. Cloud Map health tracking (post-API-Gateway migration, `infrastructure/modules/ecs-service/main.tf`) is based purely on ECS task state (`RUNNING`/`STOPPED`), not any HTTP endpoint poll — the module's own comment flags a container-level `healthCheck` block as a known, not-yet-implemented future improvement tied to ADR-026's consequences. So today, nothing depends on `/actuator/health`'s aggregate status for routing decisions either way. The `liveness`/`readiness` groups built in this PR are still legitimate forward-looking groundwork (exactly what that future improvement would consume), but this PR does not wire them into anything live. This PR's only Terraform change is adding `SPRING_DATA_REDIS_URL` to auth-workspace's ECS task `secrets` block (from the `redis_url` SSM parameter, ADR-030), so the app actually receives the connection string in AWS
