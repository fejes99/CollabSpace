...
Infrastructure live in AWS dev (`eu-central-1`): VPC, ECS cluster, ALB, security groups, IAM roles, CloudWatch log groups. `auth-workspace` and `document-service` deployed to ECS Fargate via GitHub Actions CI — health endpoints return `200 OK` at the ALB DNS name. Three service skeletons not yet started: realtime-service, ai-assistant, notification.
