...
Infrastructure live in AWS dev (`eu-central-1`): VPC, ECS cluster, ALB, security groups, IAM roles, CloudWatch log groups. `auth-workspace` and `document-service` deployed to ECS Fargate via GitHub Actions CI — health endpoints return `200 OK` at the ALB DNS name. `realtime-service` scaffold complete (lint + tests pass, Dockerfile present) — CI/CD and Terraform wiring pending. Two service skeletons not yet started: ai-assistant, notification.
