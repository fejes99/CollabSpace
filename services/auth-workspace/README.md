...
- `POST /v1/auth/logout` — protected route. Deletes the caller's refresh token row (if the presented cookie matches one) and blocklists the current access token's `jti` in Redis for the remainder of its 15-minute lifetime, derived from `X-JWT-Iat` (fails open — a Redis outage still returns `200`, just without the blocklist write). Always clears the `refresh_token` cookie. Returns `200` with no body for every case except an outright missing/malformed identity — including no cookie, an already-consumed cookie, and a genuine DB failure while deleting the row (`500`). A **repeat call with the same access token is rejected `401` `auth/token-revoked`** by `JwtBlocklistFilter` before it reaches this endpoint at all — the first call already blocklisted that `jti` — so logout is idempotent in effect but not in status code. `401` `auth/malformed-identity-headers` if `X-JWT-Jti`/`X-JWT-Iat` are missing despite the caller otherwise being authenticated (this is the first endpoint to bind those headers as required parameters directly, rather than tolerating their absence the way the filters do). See [docs/03-services/auth-workspace/plans/logout.md](../../docs/03-services/auth-workspace/plans/logout.md).
